...
(
	select category_id, c.name as category_film, city,
	sum(round(Extract(epoch from return_date-rental_date)/3600, 2)) as hours
	from category c
	...
	having sum(round(Extract(epoch from return_date-rental_date)/3600, 2)) 
	 is not null
)
 select * from (select * from category_city_hours
where city ilike 'A%'
group by category_id, category_film ,city, hours
order by hours desc
limit 1) as cities_A
union all
select * from (select * from category_city_hours
